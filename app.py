...
file_path = os.path.join(BASE_DIR, "data.txt")  # fix typo: data,txt → data.txt
...
def suggest():
    ...
    similarities = [1 - textdistance.Jaccard(qval=2).distance(w, keyword) 
    ...
    df = df[df['Similarity'] > 0.05]  # filter very low similarity
    df = df[abs(df['Words'].str.len() - len(keyword)) <= 3]  # filter by length

    # Handle no similar words
    if df.empty:
